...
# Vim
COPY .vimrc $HOME
RUN git clone https://github.com/VundleVim/Vundle.vim.git $HOME/.vim/bundle/Vundle.vim && vim +PluginInstall +qall
...
# k -> ls
RUN echo "export alias ls=k" >> $HOME/.zshrc

# 한글 사용
ENV LC_ALL=C.UTF-8

# FZF
# 여기 이후에는 이상하게 .zshrc가 써지지 않는다.
